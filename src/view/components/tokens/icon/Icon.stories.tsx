...
const CATEGORIES = {
    system: ['CartIcon', 'EditIcon', 'HeartIcon', 'MenuIcon', 'NoneIcon', 'PlusMinusIcon', 'SearchIcon', 'ShoppingBagIcon', 'TransferIcon', 'XIcon',],
...
export const AllIcons = () => {
    ...
    const filtered = Object.entries(Icons).filter(([name]) => {
        ...
        const matchesCategory =
            category === 'all' || CATEGORIES[category]?.includes(name);
    ...
    return (
        <div className="space-y-4 p-6">
            ...
            <div className="grid grid-cols-6 gap-6">
                {filtered.map(([name, Icon]) => (
                    <div key={name} className="flex flex-col items-center space-y-1 text-center">
                        <Icon className="w-6 h-6 text-gray-700 hover:text-primary transition" />
            ...
            {filtered.length === 0 && (
                <div className="text-sm text-gray-400 text-center">No icons found.</div>
            )}
        </div>
    );
};